/*
NOTE: THIS DEPENDS ON Particle Serial to send messages to the cloud, this may not be working yet, check project.
You could also use Particle.publish which currently works.
This could be expanded greatly to use higher speeds/more RAM as this sketch was designed for the Digispark Pro originally
...
void setup() {
  ...
  delay(1000);//We need a delay before we init the GPS module
...
void loop() {
  ...
  if(updateGPS()){ //this will return true if it was able to get a valid string from the GPS
    Particle.println(nmeaString); //the whole string from the GPS unit

    
    Particle.println(getStatus()); //A means we are good to go, V means it is still getting a fix
    if(gpsDateTimeReady){ //date and time are oftenr eady before the rest of the data, so if we are just looking for that we can use it much sooner
     Particle.println(getTime());
     Particle.println(getDate());
    }
    if(gpsReady){
     Particle.println(getLat());
     Particle.println(getLon());
    }
  }
  delay(1000);
