...
uart_t* uart_init(int uart_nr, int baudrate, byte config, byte mode) {
    ...
    switch(uart->uart_nr) {
        case UART0:
            ...
            if(uart->rxEnabled) pinMode(esp8266_gpioToPin[uart->rxPin], SPECIAL);
            if(uart->txEnabled) pinMode(esp8266_gpioToPin[uart->txPin], SPECIAL);
            IOSWAP &= ~(1 << IOSWAPU0);
            break;
        case UART1:
...
void uart_uninit(uart_t* uart) {
    ...
    switch(uart->rxPin) {
        case 3:
            ...
            break;
        case 13:
            pinMode(esp8266_gpioToPin[13], INPUT);
            break;
    }

    switch(uart->txPin) {
        case 1:
            pinMode(esp8266_gpioToPin[1], INPUT);
            break;
        case 2:
    ...
    switch(uart->uart_nr) {
        case UART0:
            if((uart->txPin == 1 && uart->txEnabled) || (uart->rxPin == 3 && uart->rxEnabled)) {
                if(uart->txEnabled) pinMode(esp8266_gpioToPin[15], FUNCTION_4); //TX
                if(uart->rxEnabled) pinMode(esp8266_gpioToPin[13], FUNCTION_4); //RX
                IOSWAP |= (1 << IOSWAPU0);
                ...
                if(uart->txEnabled) pinMode(esp8266_gpioToPin[1], SPECIAL); //TX
                if(uart->rxEnabled) pinMode(esp8266_gpioToPin[3], SPECIAL); //RX
                IOSWAP &= ~(1 << IOSWAPU0);
                if(uart->txEnabled){ //TX
                  pinMode(esp8266_gpioToPin[15], INPUT);
                  uart->txPin = 1;
                ...
                if(uart->rxEnabled){ //RX
                  pinMode(esp8266_gpioToPin[13], INPUT); //RX
                  uart->rxPin = 3;
                }
            }
